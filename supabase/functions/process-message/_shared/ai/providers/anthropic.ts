...
export class AnthropicProvider extends BaseAIProvider {
  ...
  protected formatMessages(
    ...
    // 🔥 FIX: Claude's API requires STRICT user/assistant alternation — it
    // rejects requests with two consecutive same-role messages (400, "roles
    // must alternate"). Multiple tool_results (one per tool call) and any
    // trailing instruction text must live in ONE user message as multiple
    // content blocks, not as separate consecutive messages. This helper
    // appends a content block to the previous message if it was already a
    // 'user' turn, instead of pushing a brand-new message object.
    const pushUserBlock = (block: { type: string; [key: string]: any }) => {
    ...
    for (const msg of history) {
      ...
      // Regular user messages — merge into the previous user turn if needed
      else if (msg.role === 'user') {
        pushUserBlock({ type: 'text', text: msg.content });
      }
    ...
    // Add new user message if present — merges into the last tool_result/
    // user turn instead of creating a second consecutive 'user' message
    if (newMessage && newMessage.trim().length > 0) {
      pushUserBlock({ type: 'text', text: newMessage });
    }
