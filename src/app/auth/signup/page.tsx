...
export default function SignUpPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    // On success, the browser is redirected away to Google — no further
    // action needed here.
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // No email link required — sign in immediately on the same screen
      // and drop the user straight into onboarding.
      ...
      if (signInError) {
        // Falls back gracefully if the Supabase project still has
        // "Confirm email" turned on — the account was created, but a
        // confirmation email was sent instead of an active session.
        if (signInError.message.toLowerCase().includes('confirm')) {
          setInfo(`Account created. We sent a confirmation link to ${formData.email} — click it, then sign in.`);
        } else {
        ...
        <div className="w-full max-w-md">
          ...
          {info ? (
            <div className="bg-green-50 border border-green-100 rounded-xl px-4 py-3 mb-4">
              ...
              <Link href="/auth/login" className="inline-block mt-3 text-sm font-medium text-blue-600 hover:text-blue-700">
                Go to sign in →
              </Link>
            </div>
          ) : (
            <>
              <button
                ...
                className="w-full flex items-center justify-center gap-3 py-3 bg-white border border-gray-200 rounded-xl font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all text-sm"
              >
                {googleLoading ? (
